data modify storage sae:runtime option set value {id:"minecraft:breeze_rod",name:{text:"Sweeping Edge",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Breeze Rod",color:"gray",italic:false}],data:{action:"enchant_option",option:"sweeping_edge"}}
scoreboard players set #available sae.tmp 1
scoreboard players set #current sae.tmp 0
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:sweeping_edge"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.name set value {text:"Sweeping Edge I",color:"aqua",italic:false}
execute if score #next sae.tmp matches 2 run data modify storage sae:runtime option.name set value {text:"Sweeping Edge II",color:"aqua",italic:false}
execute if score #next sae.tmp matches 3 run data modify storage sae:runtime option.name set value {text:"Sweeping Edge III",color:"aqua",italic:false}
