data modify storage sae:runtime option set value {id:"minecraft:ender_pearl",name:{text:"Loyalty",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Ender Pearl",color:"gray",italic:false}],data:{action:"enchant_option",option:"loyalty"}}
scoreboard players set #available sae.tmp 1
scoreboard players set #current sae.tmp 0
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:loyalty"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.name set value {text:"Loyalty I",color:"aqua",italic:false}
execute if score #next sae.tmp matches 2 run data modify storage sae:runtime option.name set value {text:"Loyalty II",color:"aqua",italic:false}
execute if score #next sae.tmp matches 3 run data modify storage sae:runtime option.name set value {text:"Loyalty III",color:"aqua",italic:false}
...
execute if data block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:channeling" run data modify storage sae:runtime option.lore append value {text:"Conflicts with Channeling",color:"red",italic:false}
execute if data block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:channeling" run scoreboard players set #available sae.tmp 0
