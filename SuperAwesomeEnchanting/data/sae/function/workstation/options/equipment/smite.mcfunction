data modify storage sae:runtime option set value {id:"minecraft:glowstone",name:{text:"Smite",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Glowstone",color:"gray",italic:false}],data:{action:"enchant_option",option:"smite"}}
scoreboard players set #available sae.tmp 1
scoreboard players set #current sae.tmp 0
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:smite"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.name set value {text:"Smite I",color:"aqua",italic:false}
execute if score #next sae.tmp matches 2 run data modify storage sae:runtime option.name set value {text:"Smite II",color:"aqua",italic:false}
execute if score #next sae.tmp matches 3 run data modify storage sae:runtime option.name set value {text:"Smite III",color:"aqua",italic:false}
execute if score #next sae.tmp matches 4 run data modify storage sae:runtime option.name set value {text:"Smite IV",color:"aqua",italic:false}
execute if score #next sae.tmp matches 5 run data modify storage sae:runtime option.name set value {text:"Smite V",color:"aqua",italic:false}
