data modify storage sae:runtime option set value {id:"minecraft:spectral_arrow",name:{text:"Infinity",color:"aqua",italic:false},lore:[{text:"Catalyst: 64 × Spectral Arrow",color:"gray",italic:false}],data:{action:"enchant_option",option:"infinity"}}
scoreboard players set #available sae.tmp 1
scoreboard players set #current sae.tmp 0
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:infinity"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.name set value {text:"Infinity I",color:"aqua",italic:false}
