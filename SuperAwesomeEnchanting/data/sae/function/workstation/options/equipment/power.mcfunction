data modify storage sae:runtime option set value {id:"minecraft:amethyst_block",name:{text:"Power",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Amethyst Block",color:"gray",italic:false}],data:{action:"enchant_option",option:"power"}}
scoreboard players set #available sae.tmp 1
scoreboard players set #current sae.tmp 0
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:power"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.name set value {text:"Power I",color:"aqua",italic:false}
execute if score #next sae.tmp matches 2 run data modify storage sae:runtime option.name set value {text:"Power II",color:"aqua",italic:false}
execute if score #next sae.tmp matches 3 run data modify storage sae:runtime option.name set value {text:"Power III",color:"aqua",italic:false}
execute if score #next sae.tmp matches 4 run data modify storage sae:runtime option.name set value {text:"Power IV",color:"aqua",italic:false}
execute if score #next sae.tmp matches 5 run data modify storage sae:runtime option.name set value {text:"Power V",color:"aqua",italic:false}
