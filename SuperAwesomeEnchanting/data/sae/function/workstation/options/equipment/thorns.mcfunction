data modify storage sae:runtime option set value {id:"minecraft:pointed_dripstone",name:{text:"Thorns",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Pointed Dripstone",color:"gray",italic:false}],data:{action:"enchant_option",option:"thorns"}}
scoreboard players set #available sae.tmp 1
scoreboard players set #current sae.tmp 0
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:thorns"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.name set value {text:"Thorns I",color:"aqua",italic:false}
execute if score #next sae.tmp matches 2 run data modify storage sae:runtime option.name set value {text:"Thorns II",color:"aqua",italic:false}
execute if score #next sae.tmp matches 3 run data modify storage sae:runtime option.name set value {text:"Thorns III",color:"aqua",italic:false}
