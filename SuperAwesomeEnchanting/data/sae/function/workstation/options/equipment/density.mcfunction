data modify storage sae:runtime option set value {id:"minecraft:gold_block",name:{text:"Density",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Gold Block",color:"gray",italic:false}],data:{action:"enchant_option",option:"density"}}
scoreboard players set #available sae.tmp 1
scoreboard players set #current sae.tmp 0
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:density"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.name set value {text:"Density I",color:"aqua",italic:false}
execute if score #next sae.tmp matches 2 run data modify storage sae:runtime option.name set value {text:"Density II",color:"aqua",italic:false}
execute if score #next sae.tmp matches 3 run data modify storage sae:runtime option.name set value {text:"Density III",color:"aqua",italic:false}
execute if score #next sae.tmp matches 4 run data modify storage sae:runtime option.name set value {text:"Density IV",color:"aqua",italic:false}
execute if score #next sae.tmp matches 5 run data modify storage sae:runtime option.name set value {text:"Density V",color:"aqua",italic:false}
