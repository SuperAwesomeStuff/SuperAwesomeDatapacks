data modify storage sae:runtime option set value {id:"minecraft:piston",name:{text:"Punch",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Piston",color:"gray",italic:false}],data:{action:"enchant_option",option:"punch"}}
scoreboard players set #available sae.tmp 1
scoreboard players set #current sae.tmp 0
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:punch"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.name set value {text:"Punch I",color:"aqua",italic:false}
execute if score #next sae.tmp matches 2 run data modify storage sae:runtime option.name set value {text:"Punch II",color:"aqua",italic:false}
