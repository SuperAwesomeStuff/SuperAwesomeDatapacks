data modify storage sae:runtime option set value {id:"minecraft:redstone_block",name:{text:"Quick Charge",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Redstone Block",color:"gray",italic:false}],data:{action:"enchant_option",option:"quick_charge"}}
scoreboard players set #available sae.tmp 1
scoreboard players set #current sae.tmp 0
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:quick_charge"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.name set value {text:"Quick Charge I",color:"aqua",italic:false}
execute if score #next sae.tmp matches 2 run data modify storage sae:runtime option.name set value {text:"Quick Charge II",color:"aqua",italic:false}
execute if score #next sae.tmp matches 3 run data modify storage sae:runtime option.name set value {text:"Quick Charge III",color:"aqua",italic:false}
