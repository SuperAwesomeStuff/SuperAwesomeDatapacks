data modify storage sae:runtime option set value {id:"minecraft:sculk_sensor",name:{text:"Swift Sneak",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Sculk Sensor",color:"gray",italic:false}],data:{action:"enchant_option",option:"swift_sneak"}}
scoreboard players set #available sae.tmp 1
scoreboard players set #current sae.tmp 0
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:swift_sneak"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.name set value {text:"Swift Sneak I",color:"aqua",italic:false}
execute if score #next sae.tmp matches 2 run data modify storage sae:runtime option.name set value {text:"Swift Sneak II",color:"aqua",italic:false}
execute if score #next sae.tmp matches 3 run data modify storage sae:runtime option.name set value {text:"Swift Sneak III",color:"aqua",italic:false}
