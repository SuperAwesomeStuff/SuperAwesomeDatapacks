data modify storage sae:runtime option set value {id:"minecraft:turtle_scute",name:{text:"Respiration",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Turtle Scute",color:"gray",italic:false}],data:{action:"enchant_option",option:"respiration"}}
scoreboard players set #available sae.tmp 1
scoreboard players set #current sae.tmp 0
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:respiration"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.name set value {text:"Respiration I",color:"aqua",italic:false}
execute if score #next sae.tmp matches 2 run data modify storage sae:runtime option.name set value {text:"Respiration II",color:"aqua",italic:false}
execute if score #next sae.tmp matches 3 run data modify storage sae:runtime option.name set value {text:"Respiration III",color:"aqua",italic:false}
