data modify storage sae:runtime option set value {id:"minecraft:magma_cream",name:{text:"Fire Protection",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Magma Cream",color:"gray",italic:false}],data:{action:"enchant_option",option:"fire_protection"}}
scoreboard players set #available sae.tmp 1
scoreboard players set #current sae.tmp 0
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:fire_protection"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.name set value {text:"Fire Protection I",color:"aqua",italic:false}
execute if score #next sae.tmp matches 2 run data modify storage sae:runtime option.name set value {text:"Fire Protection II",color:"aqua",italic:false}
execute if score #next sae.tmp matches 3 run data modify storage sae:runtime option.name set value {text:"Fire Protection III",color:"aqua",italic:false}
execute if score #next sae.tmp matches 4 run data modify storage sae:runtime option.name set value {text:"Fire Protection IV",color:"aqua",italic:false}
...
execute if data block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:protection" run data modify storage sae:runtime option.lore append value {text:"Conflicts with Protection",color:"red",italic:false}
execute if data block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:protection" run scoreboard players set #available sae.tmp 0
...
function sae:workstation/inventory/count/magma_cream
