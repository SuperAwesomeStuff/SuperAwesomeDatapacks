data modify storage sae:runtime option set value {id:"minecraft:slime_block",name:{text:"Silk Touch",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Slime Block",color:"gray",italic:false}],data:{action:"enchant_option",option:"silk_touch"}}
scoreboard players set #available sae.tmp 1
scoreboard players set #current sae.tmp 0
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:silk_touch"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.name set value {text:"Silk Touch I",color:"aqua",italic:false}
...
execute if data block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:fortune" run data modify storage sae:runtime option.lore append value {text:"Conflicts with Fortune",color:"red",italic:false}
execute if data block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:fortune" run scoreboard players set #available sae.tmp 0
