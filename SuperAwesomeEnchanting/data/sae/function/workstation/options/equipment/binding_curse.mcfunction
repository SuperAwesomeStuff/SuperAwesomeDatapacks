data modify storage sae:runtime option set value {id:"minecraft:iron_chain",name:{text:"Binding",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Iron Chain",color:"gray",italic:false}],data:{action:"enchant_option",option:"binding_curse"}}
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.name set value {text:"Binding I",color:"aqua",italic:false}
...
execute if score #available sae.tmp matches 1 if data entity @s data.sae.armed{kind:"curse",choice:"minecraft:binding_curse"} if score @s sae.timer matches 1.. run data modify storage sae:runtime option.lore[-1] set value {text:"Click again to apply curse",color:"red",italic:false}
