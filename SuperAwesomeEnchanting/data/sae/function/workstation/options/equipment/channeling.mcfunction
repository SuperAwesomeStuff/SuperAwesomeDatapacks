data modify storage sae:runtime option set value {id:"minecraft:lightning_rod",name:{text:"Channeling",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Lightning Rod",color:"gray",italic:false}],data:{action:"enchant_option",option:"channeling"}}
scoreboard players set #available sae.tmp 1
scoreboard players set #current sae.tmp 0
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:channeling"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.name set value {text:"Channeling I",color:"aqua",italic:false}
...
execute if data block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:loyalty" run data modify storage sae:runtime option.lore append value {text:"Conflicts with Loyalty",color:"red",italic:false}
execute if data block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:loyalty" run scoreboard players set #available sae.tmp 0
scoreboard players set #required sae.tmp 1
function sae:workstation/inventory/count/lightning_rod
