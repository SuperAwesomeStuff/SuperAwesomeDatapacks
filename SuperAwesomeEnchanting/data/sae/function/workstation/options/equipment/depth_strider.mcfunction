data modify storage sae:runtime option set value {id:"minecraft:nautilus_shell",name:{text:"Depth Strider",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Nautilus Shell",color:"gray",italic:false}],data:{action:"enchant_option",option:"depth_strider"}}
scoreboard players set #available sae.tmp 1
scoreboard players set #current sae.tmp 0
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:depth_strider"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.name set value {text:"Depth Strider I",color:"aqua",italic:false}
execute if score #next sae.tmp matches 2 run data modify storage sae:runtime option.name set value {text:"Depth Strider II",color:"aqua",italic:false}
execute if score #next sae.tmp matches 3 run data modify storage sae:runtime option.name set value {text:"Depth Strider III",color:"aqua",italic:false}
...
execute if data block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:frost_walker" run data modify storage sae:runtime option.lore append value {text:"Conflicts with Frost Walker",color:"red",italic:false}
execute if data block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:frost_walker" run scoreboard players set #available sae.tmp 0
