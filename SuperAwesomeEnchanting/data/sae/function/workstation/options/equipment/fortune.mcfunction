data modify storage sae:runtime option set value {id:"minecraft:diamond_block",name:{text:"Fortune",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Diamond Block",color:"gray",italic:false}],data:{action:"enchant_option",option:"fortune"}}
scoreboard players set #available sae.tmp 1
scoreboard players set #current sae.tmp 0
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:fortune"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.name set value {text:"Fortune I",color:"aqua",italic:false}
execute if score #next sae.tmp matches 2 run data modify storage sae:runtime option.name set value {text:"Fortune II",color:"aqua",italic:false}
execute if score #next sae.tmp matches 3 run data modify storage sae:runtime option.name set value {text:"Fortune III",color:"aqua",italic:false}
...
execute if data block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:silk_touch" run data modify storage sae:runtime option.lore append value {text:"Conflicts with Silk Touch",color:"red",italic:false}
execute if data block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:silk_touch" run scoreboard players set #available sae.tmp 0
scoreboard players set #required sae.tmp 1
function sae:workstation/inventory/count/diamond_block
