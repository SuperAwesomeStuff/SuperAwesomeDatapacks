data modify storage sae:runtime option set value {id:"minecraft:blaze_rod",name:{text:"Flame",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Blaze Rod",color:"gray",italic:false}],data:{action:"enchant_option",option:"flame"}}
scoreboard players set #available sae.tmp 1
scoreboard players set #current sae.tmp 0
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:flame"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.name set value {text:"Flame I",color:"aqua",italic:false}
