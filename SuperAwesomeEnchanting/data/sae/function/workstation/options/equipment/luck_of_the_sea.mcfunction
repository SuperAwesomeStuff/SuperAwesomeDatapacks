data modify storage sae:runtime option set value {id:"minecraft:pufferfish",name:{text:"Luck Of The Sea",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Pufferfish",color:"gray",italic:false}],data:{action:"enchant_option",option:"luck_of_the_sea"}}
scoreboard players set #available sae.tmp 1
scoreboard players set #current sae.tmp 0
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:luck_of_the_sea"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.name set value {text:"Luck Of The Sea I",color:"aqua",italic:false}
execute if score #next sae.tmp matches 2 run data modify storage sae:runtime option.name set value {text:"Luck Of The Sea II",color:"aqua",italic:false}
execute if score #next sae.tmp matches 3 run data modify storage sae:runtime option.name set value {text:"Luck Of The Sea III",color:"aqua",italic:false}
