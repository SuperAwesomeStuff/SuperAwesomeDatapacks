data modify storage sae:runtime option set value {id:"minecraft:dispenser",name:{text:"Multishot",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Dispenser",color:"gray",italic:false}],data:{action:"enchant_option",option:"multishot"}}
scoreboard players set #available sae.tmp 1
scoreboard players set #current sae.tmp 0
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:multishot"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.name set value {text:"Multishot I",color:"aqua",italic:false}
...
execute if data block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:piercing" run data modify storage sae:runtime option.lore append value {text:"Conflicts with Piercing",color:"red",italic:false}
execute if data block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:piercing" run scoreboard players set #available sae.tmp 0
scoreboard players set #required sae.tmp 1
function sae:workstation/inventory/count/dispenser
