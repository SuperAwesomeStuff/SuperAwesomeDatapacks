data modify storage sae:runtime option set value {id:"minecraft:obsidian",name:{text:"Blast Protection",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Obsidian",color:"gray",italic:false}],data:{action:"enchant_option",option:"blast_protection"}}
scoreboard players set #available sae.tmp 1
scoreboard players set #current sae.tmp 0
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:blast_protection"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.name set value {text:"Blast Protection I",color:"aqua",italic:false}
execute if score #next sae.tmp matches 2 run data modify storage sae:runtime option.name set value {text:"Blast Protection II",color:"aqua",italic:false}
execute if score #next sae.tmp matches 3 run data modify storage sae:runtime option.name set value {text:"Blast Protection III",color:"aqua",italic:false}
execute if score #next sae.tmp matches 4 run data modify storage sae:runtime option.name set value {text:"Blast Protection IV",color:"aqua",italic:false}
...
function sae:workstation/inventory/count/obsidian
