data modify storage sae:runtime option set value {id:"minecraft:slime_block",name:{text:"Feather Falling",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Slime Block",color:"gray",italic:false}],data:{action:"enchant_option",option:"feather_falling"}}
scoreboard players set #available sae.tmp 1
scoreboard players set #current sae.tmp 0
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:enchantments"."minecraft:feather_falling"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.name set value {text:"Feather Falling I",color:"aqua",italic:false}
execute if score #next sae.tmp matches 2 run data modify storage sae:runtime option.name set value {text:"Feather Falling II",color:"aqua",italic:false}
execute if score #next sae.tmp matches 3 run data modify storage sae:runtime option.name set value {text:"Feather Falling III",color:"aqua",italic:false}
execute if score #next sae.tmp matches 4 run data modify storage sae:runtime option.name set value {text:"Feather Falling IV",color:"aqua",italic:false}
