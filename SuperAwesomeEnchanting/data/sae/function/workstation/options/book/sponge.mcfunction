data modify storage sae:runtime option set value {id:"minecraft:sponge",name:{text:"Aqua Affinity",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Sponge",color:"gray",italic:false}],data:{action:"enchant_option",book:1b,catalyst:"sponge"}}
...
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:stored_enchantments"."minecraft:aqua_affinity"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.lore append value {text:"Aqua Affinity I",color:"gray",italic:false}
...
function sae:workstation/inventory/count/sponge
