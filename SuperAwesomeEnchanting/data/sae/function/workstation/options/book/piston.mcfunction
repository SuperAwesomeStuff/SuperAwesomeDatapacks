data modify storage sae:runtime option set value {id:"minecraft:piston",name:{text:"Knockback, Punch",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Piston",color:"gray",italic:false}],data:{action:"enchant_option",book:1b,catalyst:"piston"}}
...
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:stored_enchantments"."minecraft:knockback"
...
execute unless score #current sae.tmp matches 2.. run scoreboard players set #any sae.tmp 1
execute unless score #current sae.tmp matches 2.. if score #next sae.tmp > #cost sae.tmp run scoreboard players operation #cost sae.tmp = #next sae.tmp
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.lore append value {text:"Knockback I",color:"gray",italic:false}
execute if score #next sae.tmp matches 2 run data modify storage sae:runtime option.lore append value {text:"Knockback II",color:"gray",italic:false}
scoreboard players set #current sae.tmp 0
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:stored_enchantments"."minecraft:punch"
scoreboard players operation #next sae.tmp = #current sae.tmp
scoreboard players add #next sae.tmp 1
execute if score #current sae.tmp matches 2.. run scoreboard players set #next sae.tmp 2
execute unless score #current sae.tmp matches 2.. run scoreboard players set #any sae.tmp 1
execute unless score #current sae.tmp matches 2.. if score #next sae.tmp > #cost sae.tmp run scoreboard players operation #cost sae.tmp = #next sae.tmp
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.lore append value {text:"Punch I",color:"gray",italic:false}
execute if score #next sae.tmp matches 2 run data modify storage sae:runtime option.lore append value {text:"Punch II",color:"gray",italic:false}
