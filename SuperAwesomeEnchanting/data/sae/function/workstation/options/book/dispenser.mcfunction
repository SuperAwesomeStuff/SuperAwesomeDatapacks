data modify storage sae:runtime option set value {id:"minecraft:dispenser",name:{text:"Multishot",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Dispenser",color:"gray",italic:false}],data:{action:"enchant_option",book:1b,catalyst:"dispenser"}}
...
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:stored_enchantments"."minecraft:multishot"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.lore append value {text:"Multishot I",color:"gray",italic:false}
