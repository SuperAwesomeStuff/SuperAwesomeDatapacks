data modify storage sae:runtime option set value {id:"minecraft:lightning_rod",name:{text:"Channeling",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Lightning Rod",color:"gray",italic:false}],data:{action:"enchant_option",book:1b,catalyst:"lightning_rod"}}
...
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:stored_enchantments"."minecraft:channeling"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.lore append value {text:"Channeling I",color:"gray",italic:false}
