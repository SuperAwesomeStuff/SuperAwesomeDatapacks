data modify storage sae:runtime option set value {id:"minecraft:glass",name:{text:"Vanishing",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Glass",color:"gray",italic:false}],data:{action:"enchant_option",book:1b,catalyst:"glass"}}
...
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:stored_enchantments"."minecraft:vanishing_curse"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.lore append value {text:"Vanishing I",color:"gray",italic:false}
...
execute if score #available sae.tmp matches 1 if data entity @s data.sae.armed{kind:"curse",choice:"glass"} if score @s sae.timer matches 1.. run data modify storage sae:runtime option.lore[-1] set value {text:"Click again to apply curse",color:"red",italic:false}
