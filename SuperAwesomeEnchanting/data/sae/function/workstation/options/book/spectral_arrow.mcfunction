data modify storage sae:runtime option set value {id:"minecraft:spectral_arrow",name:{text:"Infinity",color:"aqua",italic:false},lore:[{text:"Catalyst: 64 × Spectral Arrow",color:"gray",italic:false}],data:{action:"enchant_option",book:1b,catalyst:"spectral_arrow"}}
...
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:stored_enchantments"."minecraft:infinity"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.lore append value {text:"Infinity I",color:"gray",italic:false}
...
scoreboard players set #required sae.tmp 64
function sae:workstation/inventory/count/spectral_arrow
