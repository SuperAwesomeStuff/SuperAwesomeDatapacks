data modify storage sae:runtime option set value {id:"minecraft:netherite_ingot",name:{text:"Unbreakable",color:"aqua",italic:false},lore:[{text:"Catalyst: 1 × Netherite Ingot",color:"gray",italic:false}],data:{action:"enchant_option",book:1b,catalyst:"netherite_ingot"}}
...
execute store result score #current sae.tmp run data get block ~ ~ ~ Items[{Slot:10b}].components."minecraft:stored_enchantments"."minecraft:mending"
...
execute if score #next sae.tmp matches 1 run data modify storage sae:runtime option.lore append value {text:"Unbreakable I",color:"gray",italic:false}
